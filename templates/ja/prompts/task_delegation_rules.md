...
|------|------|---------------------|----------------|-----------|
| `plan_tasks` | タスクの実行投入 | `state/pending/` に作成 | `task_queue.jsonl` に登録 | 実行が必要なタスクを発見したとき |
| `delegate_task` | 部下へのタスク委譲 | 部下の `state/pending/` に作成 | 両者の `task_queue.jsonl` に登録 | 部下に任せるとき |
| `add_task` | タスクの追跡登録のみ | **作成されない（実行されない）** | `task_queue.jsonl` に登録 | 人間指示の記録、手動着手予定のタスク |
| Task tool (Sモード) | 自動ルーティング委任 | 自動選択先に作成 | 登録される | Chat パスでの簡易委任 |

**重要**: `add_task` は追跡専用であり、TaskExec による自動実行は行われない。人間からの指示は `add_task` で記録が MUST だが、実行も必要なら `plan_tasks` も併用すること。

Heartbeat・Inbox など Task tool を使わないパスでは `plan_tasks` / `delegate_task` / `add_task` を使う。

**【MUST】`state/pending/` にJSONファイルを手動で作成してはならない。** 必ず `plan_tasks` ツール経由で投入すること。`plan_tasks` は実行キューとタスクレジストリの両方に同時登録するため、追跡漏れを防げる。

## plan_tasks によるタスク投入

`plan_tasks` は実行が必要なタスクを投入する唯一の手段（部下委譲を除く）。
...
- ❌ `state/pending/` にJSONを手動作成（必ず `plan_tasks` を使うこと）
