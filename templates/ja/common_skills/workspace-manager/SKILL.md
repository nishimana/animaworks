...
自分の injection.md の ## ワークスペース セクションを更新する:
1. `read_memory_file(path="injection.md")` で現在の内容を確認
2. 既存内容を保持しつつ `## ワークスペース` セクションを追加/更新
3. `write_memory_file(path="injection.md", content=...)` で保存

記載例:
```markdown
## ワークスペース
- デフォルト: aischreiber#3af4be6e (/home/main/dev/AI-Schreiber)
```

### 部下への割り当て（スーパバイザー用）

1. ワークスペースを登録（上記）
2. 部下の injection.md に「あなたの主な作業ディレクトリ」として記載:
   - `read_memory_file(path="../{subordinate}/injection.md")`
   - 既存内容を保持しつつ `## ワークスペース` セクションを追加/更新
