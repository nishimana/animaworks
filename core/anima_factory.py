...
def create_from_md(animas_dir: Path, md_path: Path, name: str | None = None) -> Path:
    """Create an anima from an MD character-sheet file.

    The MD file is validated, then placed as ``character_sheet.md`` in the new
    anima directory.  Sections in the sheet are applied to identity.md,
    ...
    try:
        ...
        _create_status_json(anima_dir, _parse_character_sheet_info(md_content))
    except Exception:
        logger.error(
...
def _create_status_json(anima_dir: Path, info: dict[str, str]) -> None:
