...
| `animaworks init --template NAME` | テンプレートから Person を作成（非対話） |
| `animaworks init --from-md PATH [--name NAME]` | MD ファイルから Person を作成（非対話） |
| `animaworks init --blank NAME` | 空の Person を作成（非対話） |
| `animaworks init --skip-person` | インフラのみ初期化（Person 作成をスキップ） |
...
| コマンド | 説明 |
|---|---|
| `animaworks create-person [--template NAME] [--from-md PATH] [--name NAME]` | Person を新規作成 |
| `animaworks person status [PERSON]` | Person プロセスの状態表示（省略で全員） |
| `animaworks person restart PERSON` | Person プロセスを再起動 |
| `animaworks list [--local]` | 全 Person を一覧表示 |

### コミュニケーション

| コマンド | 説明 |
|---|---|
| `animaworks chat PERSON "メッセージ" [--local] [--from NAME]` | Person にメッセージを送信 |
| `animaworks send FROM TO "メッセージ" [--thread-id ID] [--reply-to ID]` | Person 間メッセージ |
| `animaworks heartbeat PERSON [--local]` | ハートビートを手動トリガー |
...
| `animaworks logs [PERSON] [--all] [--lines N] [--date YYYYMMDD]` | ログ表示 |
