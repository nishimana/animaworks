"""Unit tests for PersonRunner startup order and ping readiness."""
...
# ── PersonRunner ping readiness ───────────────────────────


class TestPersonRunnerPingReadiness:
    """Verify that ping returns 'initializing' before DigitalPerson is ready."""
    ...
    async def test_ping_returns_initializing_before_ready(self, tmp_path):
        ...
        assert result["person"] == "test-person"
    ...
    @pytest.mark.asyncio
    async def test_run_starts_ipc_before_person_init(self, tmp_path):
        """IPC server should start before DigitalPerson is constructed."""
        runner = self._make_runner(tmp_path)
        call_order: list[str] = []
        ...
        with (
            ...
            patch(
                "core.supervisor.runner.DigitalPerson",
                side_effect=mock_person_init,
            ),
        ):
            # Start run() in background, then trigger shutdown
            async def trigger_shutdown():
                # Wait for IPC + person init to complete
                for _ in range(50):
        ...
        # IPC should start BEFORE person initialization
        assert call_order == ["ipc_start", "person_init"]
