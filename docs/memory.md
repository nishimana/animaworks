...
| **D: スキルマッチ** | skills/, common_skills/ | 200トークン | descriptionベースマッチング | 「できること」のリスト想起 |
...
| イベントタイプ | アイコン | 説明 |
|---|---|---|
| `message_received` | 📩 | ユーザーからのメッセージ受信 |
| `response_sent` | 💬 | Animaからの応答送信 |
| `dm_sent` | ✉️ | 他Animaへのダイレクトメッセージ送信 |
| `dm_received` | 📨 | 他Animaからのダイレクトメッセージ受信 |
| `channel_post` | 📢 | 共有チャネルへの投稿 |
| `channel_read` | 👁 | 共有チャネルの閲覧 |
| `human_notify` | 🔔 | 人間への通知（call_human経由） |
| `tool_use` | 🔧 | 外部ツールの使用 |
| `heartbeat_start` | 💓 | ハートビート開始 |
| `heartbeat_end` | 💓 | ハートビート終了 |
| `cron_executed` | ⏰ | cronタスクの実行 |
| `memory_write` | 📝 | 記憶ファイルへの書き込み |
| `error` | ❌ | エラー発生 |

### Priming連携

`ActivityLogger.format_for_priming()` メソッドが、取得したエントリをトークンバジェット（デフォルト1300トークン）内で人間可読なタイムラインに整形する。新しいイベントを優先し、送信者やキーワードでの関連度によるスコアリングも行われる。
